...
export default (state = defaultState, action) => {
  switch (action.type) {
    ...
    case 'USER_UPDATE': {
      ...
      return { ...state, [action.payload.prop]: action.payload.value, userModifiedData: modifiedData };
    }
    ...
    case 'RATE_ALL_FOUR': {
      ...
      newState[moduleName].phases[action.payload.prop.selectedPhase - 1].workouts.forEach((workout) => {
        exerciseCounter = 0;
